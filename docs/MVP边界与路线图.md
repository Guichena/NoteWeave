...
`一个统一工作区可以同时承载问答、Wiki 维护与资料调查三种工作模式，并通过 Deep Research 按钮、右侧产物按钮栏与门控式记忆晋升机制，把复杂任务沉淀为可追溯、可复用的知识结果。`
...
- 长期 Memory 只在问答与产物生成中按需注入，不默认污染 Deep Research 主循环

MVP 阶段不追求完整自动记忆治理，但至少要证明：

- 聊天、研究过程、产物三类来源可以进入同一记忆晋升链路
- 长期 Memory 不是对话历史原样堆积
...
5. 右侧按钮栏的默认按钮集合是否锁定为报告 / 测验 / 学习指南 / FAQ
...
`资料进入工作区 -> 用户在问答 / Wiki / 调查中消费资料 -> 用户通过 Deep Research 按钮或右侧产物按钮发起重型任务 -> 系统通过 Agent / Skill / MCP 能力栈生成可追溯结果 -> 结果经过确认回写为可沉淀知识 -> 高价值内容通过门控晋升为可复用长期 Memory。`
