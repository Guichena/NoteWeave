...
说明：Prompt scene 可以区分团队 Studio 和个人生成场景，但后台任务类型仍统一使用 `ARTIFACT_GENERATE`，不要恢复 `STUDIO_GENERATION` 或 `PERSONAL_GENERATION` 旧 Task 类型。
...
| Personal Generation | Y | Y | ArticleCard / ConceptCard / SynthesisCard / MethodologyCard / Source 选择都要可追踪 |
