...
- **个人研究工作台**：面向个人学习、研究、资料整理、技术调研、内容创作和知识管理。用户可以围绕一个主题收集资料，将资料编译为结构化知识卡片，并生成研究报告、学习指南、阅读笔记、对比分析等产物；测验、答题、评分和题库能力暂缓到 Future Backlog。
...
- **个人研究工作台**：面向个人深度研究和长期知识管理，采用全量 LLM-Wiki 编译，将个人资料转化为 Article Card、Concept Card 和 Methodology Card，并在用户确认后把高价值 Artifact 沉淀为 Synthesis Card，形成个人语义知识索引，支持长期知识融合、结构化写作、学习复盘和研究产物生成。
...
个人侧 Wiki 是完整 LLM-Wiki，由 Article Card、Concept Card、Methodology Card、Synthesis Card 和它们之间的关系组成。Synthesis Card 用于承载从用户确认过的 Artifact 中二次沉淀出来的综合知识。
...
Synthesis Card 用于表示从个人 Artifact 中沉淀出来的二次知识。它不是原始资料的直接摘要，而是用户确认某个报告、学习指南、阅读笔记或对比分析具有长期价值后，从中提炼出的稳定结论。
...
summary
insights
linked_concepts      // 通过 synthesis_concept_relation 维护
evidence_quotes
card_status
created_at
updated_at
...
- 承载从 Artifact 二次沉淀的长期知识总结。
- 避免把阶段性报告、测验、简报直接写入个人 Wiki，造成概念污染。
- 作为后续 Concept Card / Methodology Card merge proposal 的中间层。
- 保留 Artifact 与 Wiki Card 的来源关系，便于回溯。
...
4. 读取用户确认过的相关 Synthesis Card。
5. 通过 Concept Card 找到对应 Article Card。
6. 回溯原文证据片段。
7. 构造 Wiki-aware Prompt。
8. 生成结构化回答、文章、复盘或方案表达。
...
POST /api/v1/team/document-uploads/init
POST /api/v1/team/document-uploads/{uploadId}/chunks
POST /api/v1/team/document-uploads/{uploadId}/complete
POST /api/v1/team/document-uploads/{uploadId}/cancel
GET  /api/v1/team/knowledge-bases/{kbId}/documents
...
POST /api/v1/team/chat-messages/{messageId}/create-wiki-draft
```
...
创建 document 记录，状态 PENDING
  ↓
创建 DOCUMENT_PROCESS task，并通过 task_outbox 投递
  ↓
Worker 解析文本
  ↓
Chunk 切片
...
也就是说，报告、学习指南、Briefing、FAQ 等不是单独模块，而是基于统一生成引擎的不同 `artifact_type`；Quiz、答题、评分和题库当前暂缓。
...
| Quiz 测验题 | Future Backlog | 当前 MVP 暂缓，不进入 DDL、API、前端路由和验收 |
...
生成结构化内容
  ↓
保存为 Artifact；用户确认后可沉淀为 Synthesis Card
```
...
#### 测验 Quiz（Future Backlog）

Quiz、测验、答题、评分、题库和错题复习暂缓。当前 MVP 不创建 Quiz 表，不提供 Quiz API，不在 Studio 暴露测验入口。
...
`QUIZ` 及答题评分相关模板暂缓到 Future Backlog。
...
artifact_type         // REPORT / STUDY_GUIDE / READING_NOTES / FAQ / BRIEFING / COMPARISON / WIKI_DRAFT
title
content
status                // GENERATING / READY / FAILED / ARCHIVED / PUBLISHED_TO_WIKI / DISTILLED_TO_PERSONAL_WIKI
created_at
updated_at
```

来源和引用不放 JSON 字段，使用关联表：
...
Quiz 相关 `quiz / quiz_question / quiz_answer_record` 暂缓到 Future Backlog，不进入当前 DDL。
...
个人 ResearchProject 与个人 Wiki 深度绑定；团队侧仍以 KnowledgeBase 为主，团队 Artifact 经确认后进入 Wiki Draft / WikiPage。
...
员工个人研究工作台
  ...
  ├── Report / Study Guide / Reading Notes / Work Prep 生成
...
> 在员工个人研究工作台中引入 Research Project 能力，支持用户围绕主题添加 PDF、网页和文本资料，并自动进入 LLM-Wiki 编译流程；基于 Article Card、Concept Card、Methodology Card 和用户确认后的 Synthesis Card 设计多任务生成引擎，支持研究报告、测验题、学习指南和知识复盘等 NotebookLM 风格内容生成。
...
系统创建异步任务
  ↓
后台调用 ReportGenerateSkill
  ↓
右侧 Studio 展示 Report Artifact
```
...
> 不是所有生成结果都应该融入 Wiki。Wiki 只沉淀长期稳定、可复用、经过确认的知识；报告、简报、学习指南等更多是阶段性产物，默认作为 Artifact 管理。

个人侧尤其不能把生成结果默认写入 Wiki。Report、Study Guide、Reading Notes 默认都是 Artifact；只有用户显式点击“沉淀到个人 Wiki”并确认提炼结果后，才允许进入 Synthesis Card；Concept Card / Methodology Card 更新必须通过后续 merge proposal 再由用户确认。
...
```text
团队侧：Artifact → WikiPage → RAG Index
个人侧：Artifact → SynthesisCard / ConceptCard / MethodologyCard → Personal Wiki Index
```

个人侧默认先走 `Artifact → SynthesisCard`。增强阶段再支持 `Artifact → Concept merge proposal → 用户确认合并` 和 `Artifact → MethodologyCard`。
...
| 用户确认后的知识总结 | 可选 | 进入 Synthesis Card |
| Artifact 提炼出的新概念 | 可选 | 以 merge proposal 形式更新 Concept Card |
| Artifact 提炼出的方法论 | 可选 | 用户确认后创建 Methodology Card |
...
#### 生成测验弹窗（Future Backlog）

测验、答题、评分和题库暂缓，当前 MVP 不提供该弹窗。
...
```text
PersonalResearchAgent：报告、学习指南、阅读笔记、对比分析
...
报告、Wiki 编译、文献导入、Artifact 生成都可能耗时较长，因此必须设计任务中心。
...
```text
Report A 在会话 1 中生成
Study Guide B 基于 Report A 在会话 2 中生成
Comparison C 在会话 3 中引用了 Report A
```
...
created_from_session_id
created_from_message_id
artifact_type        // REPORT / STUDY_GUIDE / READING_NOTES / FAQ / BRIEFING / COMPARISON / WIKI_DRAFT / WORK_PREP
title
content
...
Quiz 相关表暂缓到 Future Backlog，当前不进入 Artifact 当前类型和 DDL。
...
scope_ids_snapshot  // 仅作创建时快照，结构化范围使用 chat_session_scope
summary
...
Citation 不放在 `chat_message` JSON 字段中，使用 `message_citation` / `citation` 关联表保存。
...
5. 异步任务中心
6. Artifact 管理
7. 团队侧可选发布为 WikiPage / 个人侧可选沉淀为 SynthesisCard
8. 对话追问与引用解释
```
...
> 参考 NotebookLM 的 Workspace + Studio 体验，设计右侧 Studio 产物生成系统，将报告、学习指南、阅读笔记、FAQ、Briefing、Wiki 草稿等能力抽象为按钮触发的异步 AI Task；后端通过 TaskService 创建任务，Agent Plan 编排多个 Skill 执行，并将结果保存为独立 Artifact，支持查看、编辑、重新生成、导出；团队侧可选发布为 WikiPage，个人侧可选沉淀为 SynthesisCard。

或者：

> 区分 Wiki 知识沉淀层与 Artifact 任务产物层：Wiki 只保存长期稳定、可复用的知识，报告、学习指南、Briefing 等默认作为独立 Artifact 管理，避免将临时生成内容污染长期知识库。
...
- `ChatSession` 属于 `Space`，并通过 `chat_session_scope` 结构化绑定具体资料范围，`scope_ids_snapshot` 只作为创建时快照。
- `Artifact` 属于 `Space`，可选关联 `ResearchProject`、`ChatSession` 或 `GenerationTask`。
- `WikiPage` 主要用于团队轻量 Wiki。
- 个人侧 Wiki 不使用 `WikiPage` 作为核心，而是由 `ArticleCard / ConceptCard / MethodologyCard / SynthesisCard` 构成。
...
| personal.card | Article / Concept / Methodology / Synthesis Card 管理 |
| chat | 多会话、消息、引用展示 |
...
- 个人侧 MVP 只做 `Artifact → SynthesisCard`，暂不自动改已有 ConceptCard，避免污染长期索引。
- Artifact 与 WikiPage / Personal Wiki Card 都必须保留关联关系，便于追踪来源。
...
> 设计并实现 NoteWeave AI 知识工作台，支持团队知识检索与个人深度研究双模式。团队侧采用大规模 RAG 架构，融合权限过滤、BM25/向量/Wiki 多路召回与 Weighted RRF 排序，支持带引用的文档问答和高价值答案 Wiki 沉淀；个人侧借鉴 LLM-Wiki 思路，将个人资料全量编译为 Article Card、Concept Card 和 Methodology Card，并将用户确认后的高价值 Artifact 沉淀为 Synthesis Card，构建可长期演化的个人语义知识索引，支撑结构化写作、知识复盘和知识融合。
...
- 设计个人 Wiki Compiler，将原始资料异步编译为 Article Card、Concept Card 和 Methodology Card，并支持用户确认后的 Artifact -> Synthesis Card 沉淀，实现跨文档概念融合与知识沉淀。
