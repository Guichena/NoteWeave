...
`Spring Boot API App + Python Research Worker + Python Artifact Worker + MySQL + Kafka + MinIO + Elasticsearch`

其中：

- `api-app` 负责接口、轻编排、状态查询、短链路同步处理
- `python-research-worker` 负责 `Deep Research` 执行
- `python-artifact-worker` 负责 `Artifact Job` 执行
...
POST /api/v2/research/runs
  ...
  -> python-research-worker 执行 Research Runtime
...
POST /api/v2/artifacts/jobs
  ...
  -> python-artifact-worker 执行 Skill Graph + Prompt Runtime
...
- 统一检索层
- Ask / Note / Wiki / Artifact / Memory 共用检索底座
...
- 优先直接基于 `OpenAI SDK` 或 `OpenAI-compatible SDK` 实现
- 系统内部只保留一层很薄的 `ModelClient` / `EmbeddingClient` 封装
- 当前阶段不单独拆独立模型服务
...
### 6.7 Built-in Capability Layer + External MCP Connector
...
- 内置能力由系统默认运行，面向所有符合权限条件的用户提供
- 用户扩展能力采用“用户自有 MCP Server 连接”模式
- 平台只保存 endpoint binding、capability binding 和策略约束
- 平台不负责托管、运维或代部署用户 MCP Server
...
Model Access Layer           执行 plan / extract / verify / write
Built-in Capability / External MCP   提供外部搜索、网页读取等能力
```
...
Model Access Layer           执行 recipe / verify / repair
Built-in Capability / External MCP   提供视频、音频、网页等外部能力
```
...
Model Access Layer           执行 normalize / compare / abstract
```
