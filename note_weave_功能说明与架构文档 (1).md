...
本节给出面向 MVP 的 MySQL 表结构设计。字段以工程落地为主，后续可根据具体 ORM、分库分表和审计需求调整。
...
常用状态字段建议使用 `VARCHAR(32)`，便于后续扩展枚举。
...
chat_session.session_type: TEAM_CHAT / PERSONAL_CHAT
artifact.status: GENERATING / READY / FAILED / ARCHIVED / PUBLISHED_TO_WIKI
generation_task.status: PENDING / RUNNING / SUCCESS / FAILED / CANCELLED
```
...
Citation 用于记录回答、Artifact、Wiki 页面中引用的来源片段。

---

### 22.9 Quiz 扩展表

如果测验需要支持答题和评分，建议拆独立表。
...
```http
POST /api/v1/team/knowledge-bases/{kbId}/documents
...
说明：上传后创建 `document` 和 `generation_task / index_task`，后台异步解析和索引。

#### 文档列表

```http
GET /api/v1/team/knowledge-bases/{kbId}/documents?page=1&pageSize=20&status=INDEXED
```
...
```http
GET /api/v1/team/spaces/{spaceId}/wiki-pages?status=PUBLISHED
```
...
说明：发布后写入 `wiki_page_version`，并触发 WikiPage 索引任务。
...
{
  "title": "RAG 技术调研",
  "description": "围绕 RAG 架构、检索优化和评估方法整理资料",
...
```http
GET /api/v1/personal/research-projects/{projectId}/concept-cards?keyword=RAG
```
...
```json
{
  ...
  "taskType": "REPORT_GENERATION_TASK",
...
```http
GET /api/v1/spaces/{spaceId}/artifacts?type=REPORT&status=READY&page=1&pageSize=20
```
...
```http
GET /api/v1/tasks?status=RUNNING&page=1&pageSize=20
```
