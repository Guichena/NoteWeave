...
public class HybridRetriever {
    ...
    public HybridRetrievalResult retrieve(TeamRetrievalQuery query, RetrievalMode requestedMode) {
        ...
        if (mode == RetrievalMode.BM25) {
            return new HybridRetrievalResult(mode, bm25Hits, bm25Hits.size(), 0, bm25Hits.size(), false, traceJson(bm25Hits, List.of(), List.of(), bm25Hits));
        }
        ...
        List<RetrievalHit> fused = vectorHits.isEmpty() && wikiHits.isEmpty()
                ? bm25Hits
                : fusion.fuse(
        ...
        return new HybridRetrievalResult(mode, fused, bm25Hits.size(), vectorHits.size(), fused.size(), fallbackUsed, traceJson(bm25Hits, vectorHits, wikiHits, fused));
    }
    ...
    private String traceJson(List<RetrievalHit> bm25Hits, List<RetrievalHit> vectorHits, List<RetrievalHit> wikiHits, List<RetrievalHit> fusedHits) {
        return """
                {"bm25":%d,"vector":%d,"wiki":%d,"fusion":%d}
                """.formatted(bm25Hits.size(), vectorHits.size(), wikiHits.size(), fusedHits.size());
    }
