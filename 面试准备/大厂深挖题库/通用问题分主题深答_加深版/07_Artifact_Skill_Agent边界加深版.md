...
把答案往下压一层：
- 业务层：面试官会追问生成结果如何保存、如何追溯、如何避免污染长期知识，以及 Skill 到底是不是 Agent。
- 架构层：StudioTaskService 创建 ARTIFACT_GENERATE 任务，ArtifactVersion 保存历史，ArtifactSource/ArtifactCitation 追踪来源，MethodologyCardService 支持预置和自定义方法论，PersonalArtifactDistillationService 用 proposal/confirm 把 Artifact 沉淀为 SynthesisCard。
